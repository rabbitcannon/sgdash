...
class FilterForm extends React.Component {
	...
	render() {
		...
		let projectStatusBoxes = _.map(this.state.project_status, (status) => {
			return (
				<li>
					<input type="checkbox" name="project_status[]" key={status.id} data-value={status.name}
								...
								<form method="post" onSubmit={this.props.updateProjects.bind(this, searchType)}>
									<div className="row expanded">
										...
										<fieldset className="large-3 columns">
											...
											<hr/>
											<ul name="project-status-list" className="column-list chks">
												{projectStatusBoxes}
											</ul>
										</fieldset>
									</div>
